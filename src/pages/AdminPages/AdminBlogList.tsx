...
export default function AdminBlogListPage() {
  ...
  const BlogCardAbsolute = ({
    ...
    <div className={`w-full max-w-[660.02px] ${layout.heightClass} relative ${cardBg} rounded-[20px] shadow-[0px_0px_0px_0.5px_rgba(0,0,0,0.03)] shadow-[0px_5px_22px_0px_rgba(0,0,0,0.04)] overflow-hidden`}>
      ...
      <div className={`left-[24px] right-[24px] ${layout.titleTop} absolute ${textSub} text-2xl font-bold font-['Plus_Jakarta_Sans'] leading-7 whitespace-normal break-words overflow-hidden max-h-[3.5rem]`}>{title}</div>
      {layout.secondTitle && layout.secondTitleTop && (
        <div className={`left-[23.99px] right-[24px] ${layout.secondTitleTop} absolute ${textSub} text-2xl font-bold font-['Plus_Jakarta_Sans'] leading-7 whitespace-normal break-words overflow-hidden max-h-[3.5rem]`}>{layout.secondTitle}</div>
      )}
      <div className={`left-[24px] right-[24px] h-12 ${layout.descTop} absolute overflow-hidden`}>
        <div className={`h-11 left-0 right-0 top-[2.22px] absolute justify-center ${textMuted} text-base font-normal font-['Inter'] leading-normal`} dangerouslySetInnerHTML={{ __html: desc }} />
      </div>
        ...
        <div className="flex w-full min-h-full">
          ...
          <main className="flex-1 p-6 flex flex-col gap-6">
            ...
            <div className="flex items-center justify-center gap-6 w-full">
              <button className={`h-11 px-4 rounded-xl inline-flex items-center gap-2 ${isDark ? 'bg-transparent' : 'bg-transparent'}`}>
                <span className={`text-sm font-semibold ${isDark ? 'text-gray-400' : 'text-gray-900/40'}`}>Newer</span>
              </button>
              <button className={`h-11 px-4 rounded-xl inline-flex items-center gap-2`}>
            ...
            {/* Newsletter */}
            <div className={`w-full h-96 ${cardBg} rounded-[20px] shadow-[0px_6px_30px_0px_rgba(0,0,0,0.08)] relative overflow-hidden`}>
